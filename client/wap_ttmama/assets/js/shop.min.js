...
(function(){if(0<$("#J_mask").length)l.mask=$("#J_mask"),l.hide=$("#J_dialog_hide"),l.mask.show();else{l.mask=$('<div id="J_mask"></div>');l.hide=$('<div id="J_dialog_hide" style="display:none"></div>');var c=Math.max(e,r);l.mask.css({width:p,height:c,'background':'rgba(0,0,0,0.7)','position':'absolute','top':'0','left':'0'});l.mask.appendTo($(document.body));l.hide.appendTo($(document.body))}"window"==l.options.type&&(l.headEle=$('<div class="dialog-head"><h3></h3></div>'),l.closeEle=$('<span class="close">X</div>').on("touchend",function(c){c.preventDefault()}).on("tap",
