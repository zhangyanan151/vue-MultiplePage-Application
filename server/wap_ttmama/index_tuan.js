...
function index(req, res) {
  var fileName = env_1.config.PATH_WAP_TTMAMA + '/static/index.html';
  var jsonName = env_1.config.PATH_WAP_TTMAMA + '/static/index.json';
  fs.exists(fileName, (exists)=>{
    if(exists) {
      fs.exists(jsonName, (exists)=>{
        if(exists) {
            res.render('index',{
            server_html: fs.readFileSync(env_1.config.PATH_WAP_TTMAMA + '/static/index.html', 'utf-8'),
            server_data: fs.readFileSync(env_1.config.PATH_WAP_TTMAMA + '/static/index.json', 'utf-8')
          });
          console.log('静态页面');
        }else {
...
function activityData(req, res) {
  ...
  request(promitOptions, function (err, resp, body) {
    ...
    request(traileroptions, function (err, resp, body) {
      ...
      timeDiff(Vue);
      var fileStr = fs.readFileSync(env_1.config.PATH_WAP_TTMAMA + '/Common_components/topimg/template.html', 'utf-8') + fs.readFileSync(env_1.config.PATH_WAP_TTMAMA + '/Common_components/group/template.html', 'utf-8');
      var vm = new Vue({
        template: fileStr,
        data: {
      ...
      vm.$on('vueServer.htmlReady', function (html) {
        ...
        // 存储到文件
        fs.writeFileSync(env_1.config.PATH_WAP_TTMAMA + '/static/index.html', html, 'utf8', function(err) {
          if (err) throw err;
          console.log('It\'s saved!');
        });
        fs.writeFileSync(env_1.config.PATH_WAP_TTMAMA + '/static/index.json', dataJson, 'utf8', function(err) {
          if (err) throw err;
          console.log('It\'s saved!');
        });
      });
